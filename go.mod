module github.com/TykTechnologies/tyk-email-driver

go 1.13

require (
	github.com/TykTechnologies/tyk v0.0.0-20200217182912-7c4b6fed5d85
	github.com/facebookgo/ensure v0.0.0-20200202191622-63f1cf65ac4c // indirect
	github.com/facebookgo/stack v0.0.0-20160209184415-751773369052 // indirect
	github.com/facebookgo/subset v0.0.0-20150612182917-8dac2c3c4870 // indirect
	github.com/fsnotify/fsnotify v1.4.8-0.20190308170645-11844c0959f6 // indirect
	github.com/hpcloud/tail v1.0.1-0.20180514194441-a1dbeea552b7 // indirect
	github.com/keighl/mandrill v0.0.0-20170605120353-1775dd4b3b41
	github.com/mailgun/mailgun-go v1.1.1
	github.com/sendgrid/sendgrid-go v2.0.1-0.20160202185359-44787bbef003+incompatible
	github.com/sendgrid/smtpapi-go v0.6.0 // indirect
	github.com/sirupsen/logrus v1.4.2
	github.com/sourcegraph/go-ses v0.0.0-20160405160939-6bd8d17cf7c1
	gopkg.in/alexcesaro/quotedprintable.v3 v3.0.0-20150716171945-2caba252f4dc // indirect
	gopkg.in/fsnotify/fsnotify.v1 v1.4.7 // indirect
	gopkg.in/gomail.v2 v2.0.0-20160411212932-81ebce5c23df
)
